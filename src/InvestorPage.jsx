...
import { Award, Building2, ChevronLeft, ChevronRight, Globe, Handshake, Mail, MapPin, Rocket, ShieldCheck, Target, Users, Cpu, Activity, Smartphone, Bell, BarChart3, Trophy, Medal, Settings, Link, Wrench, HeartHandshake, CheckCircle2 } from 'lucide-react';
      ...
      <nav className={`pointer-events-auto overflow-hidden border border-white/10 bg-darkblue/50 shadow-[0_8px_32px_0_rgba(0,0,0,0.45)] backdrop-blur-xl transition-all duration-500 ease-in-out ${isOpen ? 'w-full max-w-2xl rounded-3xl px-8 py-6' : 'rounded-full px-12 py-4'}`}>
        <div className="flex items-center justify-center gap-4 relative">
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a key={item.label} href={item.href} className="text-sm font-medium text-white/80 transition hover:text-white">
                {item.label}
              </a>
            ))}
          </div>
          ...
          <div className="md:hidden flex w-full items-center justify-between px-2">
            ...
            <button onClick={() => setIsOpen((value) => !value)} className="ml-4 text-white transition hover:text-sunset-skyblue">
              {isOpen ? <Rocket className="h-7 w-7 rotate-45" /> : <Rocket className="h-7 w-7" />}
            </button>
          </div>
        </div>
    ...
    <div className="relative z-10 mx-auto flex min-h-[calc(100vh-7rem)] w-full max-w-5xl flex-col items-center justify-center text-center">
      <FadeIn className="space-y-8">
        ...
        <div className="flex flex-col gap-4 pt-2 sm:flex-row sm:justify-center">
          ...
          <button onClick={onBackHome} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/20 bg-white/5 px-8 py-4 text-lg font-bold text-white transition hover:-translate-y-1 hover:bg-white/10">
            Return Home
          </button>
        </div>
      </FadeIn>
...
export default function InvestorPage({ onNavigateBack }) {
  ...
  return (
    <div className="min-h-screen bg-navy text-white selection:bg-sunset-skyblue selection:text-navy relative">
      <CursorGlow />
      <Navbar />
      <Hero onBackHome={onNavigateBack} />
      <AeroTrustSolution />
      <Market />
      <Competitions />
      <Traction />
